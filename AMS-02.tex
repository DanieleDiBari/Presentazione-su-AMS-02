...
\begin{frame}
	...
	\begin{center}
		...
		%\vspace{0.1cm}
		\includegraphics[height=0.25\paperheight]{ToF-Photo-01.png}
		...
		\begin{columns}
			...
			\begin{column}{0.74\textwidth}\justifying
				\scriptsize{\bluetextbf{Figure:} Top view of the upper (left) and lower (right) ToF paddles during an assembly test.}
			\end{column}
			\begin{column}{0.13\textwidth}\end{column}
		\end{columns}
	\end{center}
...
\begin{frame}
	...
	\framesubtitle{}
	\justifying
	The TOF system is composed by 4 layers of scintillator counters, two above and two below the AMS magnet.
	The four layers have respectively 8,8,10,8 paddles of different lengths (between 117 and 134 cm), longitudinally overlapping 
	by 0.5 cm to avoid geometrical inefficiencies. Each counter (fig. 2) is made of a 1 cm thick scintillator paddle, optically coupled 
	at both ends with photomultiplier tubes (PMTs).
	The external counters have a trapezoidal shape in order to maximize the acceptance.
\end{frame}
...
\begin{frame}
	...
	\small{
	The main parameters taken into account while designing the detector have been the following:	
	
	\textbf{Total sensitive area}
	AMS-02 has been designed to have a large acceptance for cosmic ray tracks. To match the full acceptance of the magnet, each 
	layer of the ToF system has to cover a circular area of about $1.6 m^2$.
	
	\textbf{Trigger selection}
	
	\textbf{Weight}
	Given the strong limitations in the total weight of the AMS detector, the TOF system was allotted $268 kg$ to accommodate 
	for the detector it self and for the support structure.
	
	\textbf{Power consumption}
	The TOF system was allowed to use about $150 W$ for photomultiplier tube operation and signal read-out, out of the $2 kW$ 
	electric power given by NASA to the AMS experiment on the ISS.
	
	\textbf{Time-of-flight resolution}
	A resolution in the ToF better than $180 ps$ is needed to satisfy the physics requirements. The choice 	was one centimetre 
	thick scintillator, as a compromise between the minimum thickness and the light output needed to reach 
	this resolution.
	}
