		...
		\begin{column}{0.4\textwidth}\centering
			...
			\begin{columns}
				...
				\begin{column}{0.92\textwidth}\justifying
					...
					\hyperlink{MAGNET-CompRes}{\textbf{\beamerbutton{more}}}
				\end{column}
				\begin{column}{0.04\textwidth}\end{column}
			\end{columns}
...
\begin{frame}
	...
	\small{The microstrip detectors operate at \textbf{full depletion by a punch-through technique} that permit to ground 
...
\begin{frame}
	\frametitle{Silicon Tracker}
	\framesubtitle{double-sided strip detector with integrated biassing structures and coupling capacitors.}
	\justifying
	...
	\begin{columns}
		\begin{column}{0.01\textwidth}\end{column}
		\begin{column}{0.44\textwidth}
			\includegraphics[width=\textwidth]{TRK-CapacitiveCoupling.png}		
		\end{column}
		%\begin{column}{0.01\textwidth}\end{column}
		\begin{column}{0.45\textwidth}
			\justifying
			\small{Direct and capacitive coupling of electronics to the detector. With direct coupling (left) the detector reverse bias current $I_r$ has to be absorbed by the electronics. With capacitive
coupling (right), only the AC part of the detector current reaches the electronics, while
the DC part goes into a bias circuit, here shown as a simple resistor}
		\end{column}
		%\begin{column}{0.01\textwidth}\end{column}
	\end{columns}
\end{frame}
...
%performance.
	...
	\begin{columns}
		...
		\begin{column}{0.33\textwidth}\justifying
			...
			Only every 3rd strip is connected to an electronics channel. 
...
\begin{frame}
	\frametitle{Silicon Tracker}
	\framesubtitle{double-sided strip detector}
	\justifying
	
	\small{Example of a detector with strip pith of $25 \, \mu m$ and analogue readout. The position resolution, 
	$\sigma \approx pitch/SNR$, is plotted as a function of the $SNR$:}
	
	\begin{itemize}\small{\justifying
		\item	\textbf{Top curve}: every 2nd strip is connected, one intermediate strip.
		\item	\textbf{Bottom curve}: every strip is connected to the readout electronics.
	}\end{itemize}
		
	\begin{center}
		\includegraphics[width=0.5\textwidth]{TRK-CapacitiveCoupling-05.png}
	\end{center}
...
\begin{frame}
	...
	\framesubtitle{double-sided strip detector}
	\justifying
	\footnotesize{The AMS microstrip detector results from the R\&D realized from the 1980's until the 1990's 
	to create first the ALEPH detector design, then the L3 SDM detector.}
	\vspace*{0.15cm}

	
	\begin{columns}
		\begin{column}{0.5\textwidth}\centering
			\includegraphics[width=0.9\textwidth]{TRACKER-AMS01vs02.png}
			
			%\includegraphics[width=\textwidth]{TRACKER-AMS01vs02-nside.png}
		\end{column}
		\begin{column}{0.5\textwidth}\centering
			\includegraphics[width=0.7\textwidth]{TRACKER-SensorGeom.png}
		\end{column}
	\end{columns}
	
	\vspace*{0.15cm}

	\footnotesize{For AMS-02, the detector design was upgraded to decrease as much as possible the noise transmitted to 
	the readout channel. Indeed, the long silicon modules of AMS-02 (up to 15 double-sided silicon sensors) are not common
	in particle physics experiments, due to noise limitations. This kind of configuration introduces effects usually neglected for
	smaller modules.}
\end{frame}
...
\begin{frame}
	\frametitle{Silicon Tracker}
	\framesubtitle{more about capacitive coupling of AMS}
	\justifying
	
	\scriptsize{
	In the case of the AMS detector, on the $p$-side, since the implant pitch is $27.5 \, \mu m$ while the readout pitch is  $110 \, \mu m$, between the 2 readout strips there are 3 intermediate strips separate. 
	
	\begin{columns}
		\begin{column}{0.01\textwidth}\end{column}
		\begin{column}{0.48\textwidth}\justifying
			\scriptsize{In such case (see figure), the coupling of the center strip with the closest readout strip $C_l = C_r \approx C_i / 2$ while for strips 2 and 4, the couplings are $C_l \approx C_i$ and $C_r \approx C_i / 3$. Under the condition of an analogue readout, it is then possible, by interpolation, to more precisely locate the passage of the crossing particle.}
		\end{column}
		\begin{column}{0.01\textwidth}\end{column}
		\begin{column}{0.49\textwidth}\centering
			\includegraphics[width=\textwidth]{TRK-CapacitiveCoupling-02.png}
		\end{column}
		\begin{column}{0.01\textwidth}\end{column}
	\end{columns}
	
%	\begin{center}
%		\includegraphics[height=0.25\paperheight]{TRK-CapacitiveCoupling-02.png}
%	\end{center}
	Taking into account the signals collected by the two amplifiers, it is thus possible to evaluate the original signal position, by computation of the center of gravity ($cog$): If $Q$ is the charge deposited on strip n, we define as $Q_l$ and $Q_r$ the charges transmitted to the left and right readout channels, respectively. Using the relation:
	\begin{equation*}
		\frac{Q_l}{C_l} = \frac{Q_r}{C_r}
	\end{equation*}
where $C_l$ and $C_r$ correspond to the capacitance between the intermediate strip n and the left (right) readout strip respectively. The center of gravity is then expressed as:
	\begin{equation*}
		cog \approx \frac{Q_l x_l + Q_r x_r}{Q_l x_l + Q_r x_r} = \frac{Q_l x_l + Q_r (x_l + 1)}{Q} = x_l + \frac{Q_r}{Q}
	\end{equation*}
}
%	\begin{block}{\footnotesize{Something about the center-of-gravity technique}}\justifying
%		\begin{columns}
%			\begin{column}{0.01\textwidth}\end{column}
%			\begin{column}{0.98\textwidth}\justifying
%				\footnotesize{In the case of the AMS detector, on the $p$-side, since the implant pitch is $27.5 \, \mu m$ while the 
%				readout pitch is $110 \, \mu m$, between the 2 readout strips there are 3 intermediate strips separate. 
%				In such case (see figure), the coupling of the center strip with the closest readout strip $C_l = C_r \approx C_i / 2$ 
%				while for strips 2 and 4, the couplings are $C_l \approx C_i$ and $C_r \approx C_i / 3$.
%				Under the condition of an analogue readout, it is then possible, by interpolation, to more precisely locate the passage 
