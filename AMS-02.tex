...
\begin{frame}[label=TRD-Straws]
	...
	\justifying
	The straw tubes have a diameter of 6 mm and different lengths, up to 2.2m. Their walls are made of 72 $\mu m$ a 
	double-layer kapton–aluminum foil that work as cathodes. A 30 $\mu m$ thick fine gold plated wire (tensioned with 1N) 
	in the center of the tube works as anode for the proportional chamber.
...
\begin{frame}
	\frametitle{TRD - Transition Radiation Detector}
	\framesubtitle{space qualification of the straw tubes}
	\justifying	
	...
	well as thermo-vacuum-cycle tests (between -40 and +60 $^\circ{C}$) followed again by vibration tests. No significant
...
%Space qualification tests have been carried out for eight 0.7m long straw modules.
%
%They underwent vibration tests (0.5 g sine sweep followed by 6.8 g random test followed by 0.5 g sine sweep) as well as thermo-vacuum-cycle tests (between -40 and +60 $^\circ{C}$) followed again by vibration tests. No significant changes in eigenfrequencies, gas tightness or gas gain were observed.

\begin{frame}
In total, 328 modules (overall, 5248 straws) arranged on 20 layers that are supported by a conically shaped octagon structure made of aluminum–honeycomb walls with carbon-fiber skins and bulkheads. 
	...
	\begin{columns}
		...
		\begin{column}{0.5\textwidth}
			\begin{figure}
				\centering
				\includegraphics[width=\textwidth]{TRD-Support-02.png}
				\footnotesize{\caption{ANO}}
%				\justifying An example of a TRD straw module (up) and a schematic view of its section (down),
%				 where also are drawn the longitudinal and vertical carbon fiber stiffeners used for the mechanical stabilization of the
%				 module.}}
			\end{figure}
		\end{column}
		\begin{column}{0.125\textwidth}\end{column}
	\end{columns}
The upper and lower four layers run parallel to the magnetic field, the others perpendicular to provide 3D tracking. 
	...
	\begin{columns}[t]
		...
		\begin{column}{0.425\textwidth}
			\begin{figure}
				...
				\justifying								
				\footnotesize{Coupled load modal analysis\\
									 FEC: $f_0 = 67.1 Hz$\\
									 (NASA requirement: $f_0 > 50 Hz$)}
			\end{figure}
		\end{column}
		...
		\begin{column}{0.425\textwidth}
			\begin{figure}
				...
				\justifying								
				\scriptsize{Main accelerations during shuttle 
									 start: 7.7g in x; and at landing: 8.9g
									in z.\\
									 Max. displacement 0.73mm well within elastic limits.}
			\end{figure}
		\end{column}
		\begin{column}{0.015\textwidth}\end{column}
	\end{columns}
...
%Each flight module is tested individually for wire
%tension, gas tightness, leakage current and gas gain
%homogeneity.  The mechanical precision
%below 100 mm kept the measured average gas gain variations of the integrated 328 straw modules below 0.7%
