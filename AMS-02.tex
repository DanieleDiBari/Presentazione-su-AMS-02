...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.01\textwidth}\end{column}
		\begin{column}{0.66\textwidth}\justifying
			\justifying
			...
			\begin{block}{\footnotesize{Main component of the MS}}\justifying
				\begin{columns}
					\begin{column}{0.99\textwidth}
						\vspace*{-0.25cm}
						\begin{itemize}\justifying
							\item	\footnotesize{The \bluetextit{Magnet}, which produces the magnetic field.}
							\item	\footnotesize{The \bluetextit{Silicon Tracker}, that reconstruct the tracks of charged 
										particles and measure, with an excellent resolution, the magnitude of their charges.}
						\end{itemize}
					\end{column}
					\begin{column}{0.01\textwidth}\end{column}
				\end{columns}
		...
		\begin{column}{0.3\textwidth}
			\centering
			\includegraphics[height=0.565\paperheight]{MS-Magnet_Deflection-02.png}
		\end{column}	
		\begin{column}{0.01\textwidth}\end{column}
	\end{columns}
	\vspace*{0.25cm}
	\justifying
			\scriptsize{In the \bluetextbf{figure} on the right is shown a schematic representation of a charged particle and its 
...
\begin{frame}
	\frametitle{Magnet}
	\framesubtitle{design constraints}
	\justifying
	
	\small{
	The AMS-02 magnet is a permanent magnet designed to optimize the requirements of a large, powerful and uniform dipole magnetic field of a flight-qualified and relatively light-weighted system.
	
%	To design this with a large acceptance ($\sim 1 m^2 sr$) and strong analysing power ($BL^2 = 0.15 T m^2$) we must make the following considerations:

	\begin{itemize}\justifying
		\small{
		\item The magnet will be launched into space and hence its weight must be as low as possible.
	...
	The specific weight of iron is rather large. With an iron-free structure, the effective parts of the magnet consist totally of Nd-Fe-B, which has a specific weight of $7.4 \cdot 10^3 kg/m^3$. This reduces the gross weight of the magnet.}
\end{frame}
...
%	The magnet is made of 6400 Nd-Fe-B blocks of sides 5×5×2.5 cm3. The blocks are arrayed in 100 circle shaped 64 elements layers (one layer is depicted in Fig. 2.2), constituting a cylinder of length 800 mm, inner radius r1 = 111.5 cm and outer radius r2 = 129.9 cm. The blocks are arranged to form 64 segments with varying field directions in order to produce a 0.15 T field intensity in the magnet center [6].
\end{frame}
