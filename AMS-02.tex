...
\begin{frame}
	...
	\small{The basic readout element of the Silicon Tracker is called ``\textit{ladder}'', the figures below show the two side of a 
	15 sensors ladder being assembled.}
	...
	\begin{columns}
		...
		\begin{column}{0.65\textwidth}\justifying
			...
			The energy deposit is read out, for each ladder, by 1024 readout channels: 640 on the $p$-side, 384 on the $n$-side. 
			The readout electronics is placed directly on the front end board at the end of the ladder. 
			The number of ladder composing the AMS tracking system is 192, for a total of $\sim 200,000$ readout channels and a 
			total active area of $6.74 m^2$.	}
...
\begin{frame}
	...
	\begin{block}{\footnotesize{Goals of the ladder fabrication}}
		\begin{columns}
			...
			\begin{column}{0.98\textwidth}\justifying
				...
				The alignment precision is provided by the mechanical precision of the jigs ($1 - 2 \, \mu m$) and the precision of the 
				sensor cut.}
			\end{column}
			\begin{column}{0.01\textwidth}\end{column}
		\end{columns}
	...
	The results for the sensor alignment for 125 (of 192) AMS-02 ladders are shown in Figure 2.60. A particular effort has been 
	made to maintain the low noise by passivation of the silicon and by optimization of the ladder assembly procedure.
\end{frame}
...
\begin{frame}
	...
	\begin{columns}
		...
		\begin{column}{0.52\textwidth}\justifying
			...
			The flexibility of these UFs allows the hybrids to be placed back-to-back, perpendicular to the detection plane, 
			thus minimizing the material in the sensitive region of the tracker and maximizing the active area within the 
			magnet bore. 
...
\begin{frame}
	...
	The 640 $p^+$ readout strips of each sensor of the same ladder are bonded to a short Upilex cable which brings the signal to
	the front end electronics hosted on PCBs (hybrids, see Fig. 2.9). 
	The flexibility of the cable allows to place the electronics of the inner layers at an angle of $90^\circ$ with respect to the 
	tracker plane, allowing to maximize the active area within the magnet bore. 
	
	To bring the $n^+$ readout strips signal to the electronics on the ladder edge, every i-th $n^+$ strip (for a total of 192) of
	even (odd) sensors is bonded on the same i-th channel of a long Upilex cable.
	
	The $x$ coordinate is thus reconstructed with a multiplicity that can be solved only from the complete track knowledge.

	All the 1024 (640 + 384) readout channels of a single ladder are AC coupled with 16 (10 + 6) high dynamic range Voltage
	Amplifiers (VA) trough $700 \, pF$ capacitors.
	
	The capacitors permit to suppress the high frequency components of the leakage current allowing a very low power 
	consumption of less than $1 \, mW$ per channel.
	The signal passes trough a semi-Gaussian CR-RC shaper with a $\approx 5 \, \mu s$ shaping time. 
	Such a long shaping time allows a consistent reduction of noise. 
	Then signal is sampled and sent to the Tracker Data Reduction board (see Sec. 2.2.9.1).
\end{frame}
...
\begin{frame}
	...
	\framesubtitle{the structure of the AMS-02 tracker}
	\justifying
%	\small{The Silicon Tracker is composed by 9 layers arranged along AMS-02 detector height which allow the 3D reconstruction of incoming particle's trajectory:} 
	
	\begin{columns}
		\begin{column}{0.01\textwidth}\end{column}
		\begin{column}{0.43\textwidth}\justifying
			\small{The Silicon Tracker is composed by 9 layers arranged along AMS-02 detector height which allow the 3D
			reconstruction of incoming particle's trajectory.
			The tracker power consumption is foreseen to be of the order 640 W. The tracker will weigh
186 kg.
			The total active area of $6.75 \, m^2$, for a total number of 2284 Silicon sensors arranged in 192 ladders, 
			makes the AMS-02 Silicon Tracker the largest tracker for a magnetic spectrometer ever built for space applications.}
		\end{column}
		\begin{column}{0.55\textwidth}\justifying
			\includegraphics[width=\textwidth]{TRACKER-04.png}
...
\begin{frame}
	...
	\footnotesize{Layers 1 and 9 constitute the \textbf{external tracker} and are positioned at both ends of the detector to
	maximize the lever arm in the trajectory determination: the first is above the TRD at $z =155 \, cm$ and the last is under the 
	RICH (before the ECAL) at $z = -135 \, cm$. %(In Fig.2.7 the external layers during the AMS-02 detector integration are shown)
	}
	
	\begin{columns}
		\begin{column}{0.6\textwidth}\justifying
				\footnotesize{Layers 2 to 8 constitute the \textbf{inner tracker} and are arranged just above the magnet mechanical structure 
	(layer 2, $z = 55 \, cm$) and inside the magnet bore (layers 3-8). Three carbon fibre and aluminium honeycomb planes
%(see Figs. 2.5 and 2.6) 
	support the layer pairs inside the magnet bore (3-4, 5-6,7-8), with a relative distance between layers belonging to the same
	plane of $\sim 4 \, cm$ along the z axis. 
	The most inner plane (layers 5-6) is placed at the magnet center ($z = 0$) and the two neighbours symmetrically disposed at
	$z = \pm 27 \, cm$ from the central one.}
		\end{column}
		\begin{column}{0.4\textwidth}
			\includegraphics[width=\textwidth]{TRACKER-03.png}
		\end{column}
	\end{columns}
...
\begin{frame}
Silicon wafers are arranged in mechanical and electrical
units called ladders. Each ladder consists of a support
structure and a linear arrangement of 9 to 15 sensors. Ladders
are placed on support planes to cover the AMS acceptance
at various heights in the detector. As also depicted in
...
\justifying
\small{
The AMS-02 tracker contains 2264, $41.360 \times 72.045 \times 0.300 \, mm^3$, double-sided microstrip sensors obtained from n-type, high resistivity ($> 6\,k\Omega$) circular silicon wafers with $<1 \, 1\, 1>$ lattice orientation. 
The sensors are biased via the punchthrough technique. 
The $14 \, \mu m$ wide p-side (junction) strips have implantation and readout pitches of 27.5 and $110 \, \mu m$; the
$15 \, \mu m$ wide n-side (ohmic) strips have implantation and readout pitches of 52 and $208 \, \mu m$. 
The average values of the important noise performance parameters, based on the measurements of
$\sim 1500$ AMS-02 sensors, are reported in Table 1.
}
\end{frame}
...
\begin{frame}[label=TRK-BondSchem-nside,noframenumbering]
	...
	\begin{center}
		\includegraphics[width=0.95\textwidth]{TRACKER-BondingScheme-nside.png}
	\end{center}
	...
	\begin{columns}
		\begin{column}{0.05\textwidth}\end{column}
		\begin{column}{0.9\textwidth}\centering
			\small{\bluetextbf{Figure:} Signal routing for the K5 design - a ladder with 12 sensors\footnotemark[1].}		
		\end{column}
		\begin{column}{0.05\textwidth}\end{column}
	\end{columns}
	...
	\footnotetext[1]{The x coordinate is reconstructed with a multiplicity that can be solved only from the complete track knowledge.}
\end{frame}
...
\begin{frame}[label=TRK-Fab01,noframenumbering]
	...
	\hyperlink{TRK-Ladder1}{\textbf{\beamerbutton{presentation}}}}
	...
	\begin{columns}
		...
		\begin{column}{0.98\textwidth}
			\begin{figure}
				...
				\hspace*{0.5cm}
				\includegraphics[height=0.52\paperheight]{TRACKER-LadFabrication-01b.png}
			\end{figure}
		\end{column}
		\begin{column}{0.02\textwidth}\end{column}
	\end{columns}
...
\begin{frame}[label=TRK-Fab07,noframenumbering]
	...
	\begin{columns}
		...
		\begin{column}{0.6\textwidth}
			\begin{figure}
				\includegraphics[width=\textwidth]{TRACKER-LadFabrication-18b.png}
			\end{figure}
		\end{column}
		\begin{column}{0.2\textwidth}\end{column}
	\end{columns}
	...
	\begin{columns}
		...
		\begin{column}{0.6\textwidth}
			\begin{figure}
				\includegraphics[height=0.27\paperheight]{TRACKER-LadFabrication-18bbb.png}
				\hfill
				\includegraphics[height=0.27\paperheight]{TRACKER-LadFabrication-18bb.png}
			\end{figure}
		\end{column}
		\begin{column}{0.2\textwidth}\end{column}
	\end{columns}
	...
	\begin{columns}[t]
		...
		\begin{column}{0.8\textwidth}
			\begin{figure}\justifying									
				...
				To connect electrically all the sensors in a ladder, all the strips on the $p$-side are daisy-chained with a 25 um Al wire through ultrasonic bondings.}
			\end{figure}
		\end{column}
		\begin{column}{0.1\textwidth}\end{column}
	\end{columns}
...
\begin{frame}[label=TRK-Fab10,noframenumbering]
	...
	\begin{columns}[t]
		...
		\begin{column}{0.46\textwidth}
			\begin{figure}\justifying									
				\scriptsize{\bluetextbf{Figure 27:} 
				The Upilex foil, then the ladder are placed on a table. 
				The foil is bent over the ladder with pushers.
				The shielding is closed with rosin-free solder.}
			\end{figure}
