...
\graphicspath{	
	...
	{./Images/TRD/}
	{./Images/ToF/}
	{./Images/Magnetic_Spectrometer/}
	{./Images/Magnetic_Spectrometer/Magnet/}
	{./Images/Magnetic_Spectrometer/Tracker/}
	{./Images/Magnetic_Spectrometer/Tracker/Ladder_Fabrication/}
}
...
\begin{frame}[label=TRD-Straws]
	...
	\justifying
	The straw tubes have a diameter of $6 \, mm$ and different lengths, up to $2 \, m$. Their walls, made of 72 $\mu m$ a 
	double-layer kapton-aluminum foil, work as cathodes. In the center of each tube, there is a 30 $\mu m$ thick fine 
	gold plated wire (tensioned with $1 \, N$) that works as anode for the proportional chamber.
...
\begin{frame}[label=TRD-SQ_StrawsMod]
	...
	\justifying	
	\small{Space qualification tests have been carried out for eight $0.7 \, m$ long straw modules.}
...
\begin{frame}
	\frametitle{Silicon Tracker}
	\framesubtitle{performance: rigidity measurements I}
	\justifying

	\small{The incoming particle rigidity is inferred by the measurement of the track curvature in the magnetic field.
	A fit to the measured crossing coordinates is applied to extract the trajectory parameters.}
	...
	\begin{columns}
		...
		\begin{column}{0.53\textwidth}
			...
			\small{Rigidity resolution can be estimated from the Monte Carlo simulation (MC) once the point resolution and the 
			effects of the residual misalignment are correctly simulated.
			
			\vspace*{0.15cm}
			
			As can be seen in the figure, there is a very good agreement between the simulated point resolution and the measured 
			one. 
			Similar agreements, for both He nuclei and protons, is observed on $x$ and $y$ projections at different track
			inclinations.
			
			\vspace*{0.15cm}

			This results gives value to the MC used to obtain the rigidity resolution.}
		\end{column}
		\begin{column}{0.01\textwidth}\end{column}
		\begin{column}{0.44\textwidth}
			\centering
			\small{\bluetextit{Position Measurement Resolution}}
			
			\vspace*{0.15cm}
			
			\includegraphics[width=0.97\textwidth]{TRACKER-PosRes-Meas_VS_MC-02.png}	
			
			\vspace*{0.05cm}
			
			\justifying
			\scriptsize{\bluetextbf{Figure:} He tracks fit residual on bending coordinate for inner layers. Black points ISS
			data, red points MC}		
		\end{column}
		\begin{column}{0.01\textwidth}\end{column}
	\end{columns}
...
\begin{frame}
	\frametitle{Silicon Tracker}
	\framesubtitle{performance: rigidity measurements II}
	\justifying
	
	\vspace*{-0.1cm}

	\small{Rigidity measurement resolution for protons and He estimated from MC simulation:}
	...
	\begin{columns}
		...
		\begin{column}{0.92\textwidth}
			\centering
			\includegraphics[height=0.35\paperheight]{TRACKER-RigidityRes-01.png}	
			
%			\vspace*{0.05cm}
			
			\justifying
			\scriptsize{\bluetextbf{Figures:}  
			The rigidity measurement resolution is calculated for different tracker spans.
			Note: the Maximum Detectable Rigidity correspond to a value of resolution equal to 1.}		
		\end{column}
		\begin{column}{0.04\textwidth}\end{column}
	\end{columns}	
	...
	\small{Since the accuracy of the curvature measurements, among other parameters, strongly depend on the
	span (i.e. distance between the uppermost and the bottommost measurement of the position), the rigidity resolution 
	also depends on this parameter.
	
	\vspace*{0.15cm}

	The pictures show that the presence of external layers, used to increase the trajectory lever arm, allows to measure the 
	rigidity of traversing particles up to $TeV$.}
\end{frame}

\begin{frame}
	\frametitle{Silicon Tracker}
	\framesubtitle{performance: rigidity measurements III}
	\justifying
	
	As can be see from graphs in the previous slide, the Maximum Detectable Rigidity (MDR) in the different span configurations
	is: 
	
	\begin{table}[h]
		\centering
		\begin{tabular}{p{0.5cm}|p{1cm}|p{1cm}|p{1cm}|p{1cm}}
			\multicolumn{1}{c}{\textbf{}} & 
			\multicolumn{1}{c}{\textbf{\footnotesize{Inner Tracker}}} & 
			\multicolumn{1}{c}{\textbf{\footnotesize{Inner + Layer 1}}} & 
			\multicolumn{1}{c}{\textbf{\footnotesize{Inner + Layer 9}}} &
			\multicolumn{1}{c}{\textbf{\footnotesize{Full Tracker}}}
			\\
			\toprule
			\multicolumn{1}{r|}{\footnotesize{$p$}} & 
			\multicolumn{1}{|c|}{\footnotesize{$240 \, GV$}} &
			\multicolumn{1}{|c|}{\footnotesize{$540 \, GV$}} &
			\multicolumn{1}{|c|}{\footnotesize{$750 \, GV$}} &
			\multicolumn{1}{|c}{\footnotesize{$2000 \, GV$}} 
			\\
			\multicolumn{1}{r|}{\footnotesize{He}} & 
			\multicolumn{1}{|c|}{\footnotesize{$400 \, GV$}} &
			\multicolumn{1}{|c|}{\footnotesize{$1100 \, GV$}} &
			\multicolumn{1}{|c|}{\footnotesize{$1600 \, GV$}} &
			\multicolumn{1}{|c}{\footnotesize{$3200 \, GV$}} 
			\\
			\bottomrule	
		\end{tabular}
		...
		\scriptsize{\textit{These reported estimations are affected by a 10\% error} \cite{Ambrosi-01}\textit{.}}
	\end{table}
	
	Therefore, the greatest MDR (obtained in the maximum span configuration with the Full Tracker) is approximatively $2 \, TV$ 
	for protons and  $3.2 \, TV$ for He nuclei.
\end{frame}

\begin{frame}
	\frametitle{Silicon Tracker}
	\framesubtitle{performance: charge confusion I}
	\justifying
	
	The \textbf{charge confusion}, i.e. reconstructing a track with the wrong sign of charge, is one of the limiting factor in 
	the measurement of antiparticle abundances in cosmic rays.
	
	\begin{block}{\small{Two sources of charge confusion:}}
		\vspace*{-0.25cm}
		\begin{columns}
			\begin{column}{0.99\textwidth}
				\begin{itemize}\justifying
				\footnotesize{
					\item[$\blacktriangleright$]	\textbf{Intrinsic:} Since tracker measures the curvature with a given gaussian 
					resolution, the tails of this gaussian may extend beyond zero introducing then the probability to get a measured 
					rigidity of the wrong sign. 
					The resolution on the curvature worsen for larger rigidities as a consequence the probability of charge confusion 
					equally increases. 
					\textit{This source of charge confusion can not practically reduced but can be well estimated once the rigidity 
					resolution is well known}.

					%\vspace*{-0.1cm}
					
					\item[$\blacktriangleright$]	\textbf{Extrinsic:} arises from the particle interactions with the AMS material. 
					The scattering of a particle may create a kink in the trajectory. 
					As a consequence of the finite tracker point resolution this kink may fake the fit procedure into producing a wrong 
					sign rigidity. 
					\textit{This second class of charge confused events may be identified from the event features} \cite{Ambrosi-01}.
				}	
				\end{itemize}
			\end{column}
			\begin{column}{0.01\textwidth}	\end{column}
...
\begin{frame}
	\frametitle{Silicon Tracker}
	\framesubtitle{performance: charge confusion II}
	\justifying
	
	\small{In order to spot charge confusion events, a set of 22 variables from the data are taken in account.
	None of these variables, however, shows individually a large separation power between ``well'' and ``bad'' reconstructed 
	tracks.}
	...
	\begin{columns}
		...
		\begin{column}{0.52\textwidth}
			...
			\small{Therefore has been implemented a Boosted Decision Tree (BDT), including the mentioned 22 quantities, that was 
			trained on Monte Carlo simulated data.	
			
			\vspace*{0.15cm}
			
			The figure shows the charge confusion for electron events as observed in a MC simulation (red line) and 
			as estimated with the Tracker BDT on the data (black points).
	
			\vspace*{0.15cm}

			An overall agreement between the MC estimation and the Tracker BDT applied to the data confirms the
		...
		\begin{column}{0.45\textwidth}
			...
			\small{\bluetextit{Charge Confusion}}
			
			\vspace*{0.05cm}
			
			\includegraphics[width=0.97\textwidth]{TRACKER-ChargeConfusion-02.png}	
			
%			\vspace*{0.05cm}
			
			\justifying
			\scriptsize{\bluetextbf{Figure:} Charge confusion is presented as a function of the total electron energy as measured in 
			the ECAL, thus corresponding to a very good estimation of the true rigidity. }		
		\end{column}
		\begin{column}{0.01\textwidth}\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}
	\frametitle{Silicon Tracker}
	\framesubtitle{performance: charge measurements I}
	\justifying
	
	\small{Redundancy is key to obtain high accuracy measurements of $|Z|$ by $dE/dx$. 
	
	\vspace*{0.15cm}

	For this purpose, the specific energy loss is measured no less than 13 times along the particle trajectory. 
	Actually, each tracker hit delivers two measurements of specific energy loss (one arise from the $n$-side and the other 
	from the $p$-side), with different gain and saturation properties on the two readout sides.}
	...
	\begin{columns}
		...
		\begin{column}{0.4\textwidth}
			\centering
			\footnotesize{\bluetextit{Charge Resolution (CR)}}
		\end{column}
		\begin{column}{0.56\textwidth}\end{column}
	\end{columns}
	...
	\begin{columns}
		\begin{column}{0.04\textwidth}\end{column}
		\begin{column}{0.4\textwidth}
			\includegraphics[width=0.98\textwidth]{TRACKER-C_distrib-Calibration-01.png}			
		\end{column}
		\begin{column}{0.52\textwidth}
			\justifying
			\scriptsize{\textitem{Figure}{CR for the two sides of the tracker sensors}\\
			The figure shows, up to silicon ($Z = 14$), the two different resolution of the full tracker for the $n$-side and $p$-side 
			and the overall improvement in the tracker charge measurement after applying the calibration. 
			At high $Z$ the response of the microstrip detector starts saturating (for each side), implying a natural
			degradation of the measurement.}
		\end{column}
		\begin{column}{0.04\textwidth}\end{column}
	\end{columns}
	
%	\small{Typical charge resolutions for Carbon nuclei after calibration are 0.3 charge units per tracker layer (that correspond 
%			at a resolution of about 0.15 for the inner tracker).}
\end{frame}

\begin{frame}[label=Perf]
	\frametitle{Silicon Tracker}
	\framesubtitle{performance: charge measurements II}
	\justifying		
	
	\vspace*{-0.05cm}
	
	\small{The \textbf{charge calibration} see before consist in a procedure that allows to corrects for path length differences, the 
	front end gains, charge losses depending on impact position and angle as well as the dependence of $dE/dx$ on momentum.}
	\vspace*{0.15cm}
	...
	\begin{columns}
		...
		\begin{column}{0.52\textwidth}
			\justifying
			\small{In order to obtain the overall charge identification power, the two measurements shown in the previous slide are 
			combined using a weighted sum, where the weights are related to the specific resolutions.
			
			\vspace*{0.15cm}
			
			The \textit{figure shows the final combined estimator for the full tracker} over the full range of charges.
			
			\vspace*{0.15cm}
			
			At 99\% selection efficiency, the contamination from neighbouring charges is estimated to be lower 
			than $10^{-4}$ up 	to oxygen ($Z = 8$) with a miss-identification probability for carbon lower than $10^{-6}$.}
		\end{column}
		\begin{column}{0.01\textwidth}\end{column}
		\begin{column}{0.45\textwidth}
			\centering
			\includegraphics[width=0.97\textwidth]{TRACKER-Charge_distrib-04.png}	
			
			\vspace*{0.02cm}
			
			\justifying
			\scriptsize{\bluetextbf{Figure:} Nuclei up to Si can clearly be separated. 
			Above, nuclei with even $Z$ can also be distinguished. 
			The Fe peak itself is clearly visible due to its natural higher abundance compared to neighbouring charges.}
		\end{column}
		\begin{column}{0.01\textwidth}\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}
	\frametitle{Silicon Tracker}
	\framesubtitle{performance: charge measurements III}
	\justifying		
	...
	\begin{columns}
		...
		\begin{column}{0.5\textwidth}
			\justifying
			\small{Finally, for the combined measurement, the tracker reach a resolution at the level of 0.1 charge units ($c.u.$) for 
			carbon and lower than $0.3 \,  c.u.$ up to silicon. }
	
			\vspace*{0.15cm}
		
			\small{This is shown in the figure where the quoted values are obtained by fitting 
			single charge distributions with a simple gaussian function for two different cases: a minimum number of 8 layers entering 
			the measurement (magenta curve) and the general case with at least 4 layers (black curve).}
		\end{column}
		\begin{column}{0.48\textwidth}
			\centering
%			\hspace*{0.35cm}\tiny{}
%			
			\includegraphics[width=0.96\textwidth]{TRACKER-Resolutio-01.png}	
			
			\vspace*{0.05cm}

			\hspace*{0.35cm}\footnotesize{\bluetextit{Tracker Charge Resolution}}\\	
			\hspace*{0.35cm}\footnotesize{\bluetextit{in charge unit}}		
		\end{column}
		\begin{column}{0.01\textwidth}\end{column}
	\end{columns}	
\end{frame}
...
\begin{frame}[allowframebreaks]
	...
	\bibitem{AMS-website-Gen}
	AMS Geneva group, ``\textit{AMS - The Alpha Magnetic Spectrometer Experiment}'' 
	\url{http://dpnc.unige.ch/ams/ams_beta/Welcome_New.shtml}
